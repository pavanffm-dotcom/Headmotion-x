...
class DynamicPlanner(
    ...
    suspend fun decideNextAction(
        ...
        // Phase 20 Target 7: Avoid duplicate decisions for completed launch goals
        val isSimpleOpen = isSimpleLaunchGoal(originalGoal)
        if (targetApp != null && currentPkg.contains(targetApp.packageHint) && isSimpleOpen && missionState.lastAction?.name == CanonicalTools.LAUNCH_APP) {
            Log.i(TAG, "Fast-path: Target app '${targetApp.name}' is foregrounded for simple launch goal. Completing mission directly.")
            return ToolCall(
                name = CanonicalTools.FINISH_TASK,
                arguments = mapOf("success" to true, "spoken_summary" to "${targetApp.name} is now open, Sir."),
                thought = "${targetApp.name} opened and confirmed in foreground."
            )
        }

        // Phase 20 Target 7: Avoid duplicate decisions when web search result is already available
        if (missionState.lastAction?.name == CanonicalTools.WEB_SEARCH) {
            val searchSummary = missionState.lastActionResult?.verification ?: "Information retrieved from the web, Sir."
            return ToolCall(
                name = CanonicalTools.FINISH_TASK,
                arguments = mapOf(
                    "success" to (missionState.lastActionResult?.success ?: true),
                    "spoken_summary" to searchSummary
                ),
                thought = "Web search concluded; summarizing retrieved information to finish mission."
            )
        }
        ...
        val request = ModelDecisionRequest(
            ...
            compressedScreenIndex = currentObservation.toOptimizedSemanticIndex(goal = originalGoal, subgoal = subgoal),
    ...
    private fun isSimpleLaunchGoal(goal: String): Boolean {
        val lower = goal.lowercase().trim()
        val openPrefixes = listOf("open ", "launch ", "start ", "kholo ", "chalao ")
        val hasPrefix = openPrefixes.any { lower.startsWith(it) }
        val words = lower.split(Regex("\\s+"))
        val hasChainedActions = lower.contains(" and ") || lower.contains(" then ") || lower.contains(" send ") ||
                lower.contains(" search ") || lower.contains(" type ") || lower.contains(" write ")
        return hasPrefix && words.size <= 4 && !hasChainedActions
    }
}