...
class MissionValidationOrchestrator(
    ...
    suspend fun executeStep(
        ...
        val compressedIndex = currentScreen.toOptimizedSemanticIndex(
            goal = missionState.originalUserGoal,
            subgoal = activeGoal
        )
        val modelRequest = ModelDecisionRequest(
            ...
            compressedScreenIndex = compressedIndex,
        ...
        val tokenEstimate = ContextCompressor.estimateTokenCount(modelRequest)
        ...
        // Phase 19: Record into JarvisMissionDebugger for developer traceability
        val stepTrace = JarvisMissionDebugger.createTraceFromRecord(
            record = record,
            provider = "GEMINI",
            model = "gemini-1.5-flash"
        )
        val latest = JarvisMissionDebugger.getLatestMission()
        val targetMissionId = if (latest != null && latest.goal == missionState.originalUserGoal && latest.status == "IN_PROGRESS") {
            latest.missionId
        } else {
            JarvisMissionDebugger.startMission(missionState.originalUserGoal, "GEMINI", "gemini-1.5-flash").missionId
        }
        JarvisMissionDebugger.recordStep(targetMissionId, stepTrace)
