...
class AgentOrchestrator(
    ...
    val performanceTracker = MissionPerformanceTracker()
    ...
    var currentMissionState: MissionState? = null
        private set
    private var activeDebuggerMissionId: String? = null
    ...
    fun startMission(userGoal: String) {
        ...
        // Phase 19: Register with J.A.R.V.I.S. Mission Trace Debugger
        val activeModelName = appSettings.getActiveCustomModel()?.modelId?.takeIf { it.isNotBlank() } ?: appSettings.aiModelName
        val traceRecord = JarvisMissionDebugger.startMission(
            goal = userGoal,
            provider = appSettings.aiProvider.name,
            modelName = activeModelName
        )
        activeDebuggerMissionId = traceRecord.missionId
    ...
    private suspend fun executeMissionLoop() = withContext(Dispatchers.Default) {
        ...
        var lastPostObservation: WorldState? = null
        ...
        while (isActive && mission.status != AutonomousMissionStatus.COMPLETED && mission.status != AutonomousMissionStatus.FAILED) {
            ...
            // Phase 20 Targets 6, 8, 9: Reuse stable verified post-observation when safe to avoid duplicate screen observation
            var liveWorldState = if (lastPostObservation != null) {
                performanceTracker.recordDuplicateObservationAvoided()
                performanceTracker.recordStateReused()
                lastPostObservation!!
            } else {
                screenObserver.observeWorldState()
            }
            lastPostObservation = null // Reset so fresh observation is collected when state changes
            ...
            // Phase 20 Target 1: Adaptive Screenshot Capture evaluation
            val screenshotDecision = AdaptiveScreenshotController.evaluate(
                worldState = liveWorldState,
                isVisionSupported = isVisionModel,
                isCaptureActive = isCaptureActive,
                userGoal = mission.originalUserGoal,
                consecutiveActionFailures = mission.consecutiveFailedActions,
                isRecoveryMode = mission.consecutiveFailedActions > 0 || consecutiveUnchanged > 0
            )

            var screenshotBase64: String? = null
            if (screenshotDecision.shouldCapture) {
                Log.i(TAG, "[CAPTURE_STARTED] Adaptive screenshot capture for $activeModelName (${screenshotDecision.reason})")
                timeline.record("CAPTURE_STARTED", "Adaptive capture: ${screenshotDecision.reason}")
                try {
                    screenshotBase64 = captureMgr?.captureAdaptiveScreenshotBase64(
                        quality = screenshotDecision.recommendedQuality,
                        maxWidth = screenshotDecision.recommendedTargetWidth
                    ) ?: captureMgr?.captureScreenshotBase64()
                    if (!screenshotBase64.isNullOrBlank()) {
                        performanceTracker.recordScreenshotCapture(
                            bytes = (screenshotBase64.length * 3L) / 4L,
                            base64Chars = screenshotBase64.length
                        )
                        Log.i(TAG, "[CAPTURE_SUCCESS] Captured adaptive screenshot (${screenshotBase64.length} chars base64)")
                        timeline.record("CAPTURE_SUCCESS", "Captured adaptive frame (${screenshotBase64.length} chars)")
                    } else {
                ...
                val omitReason = screenshotDecision.reason
            ...
            if (toolCall.name == CanonicalTools.FINISH_TASK) {
                ...
                activeDebuggerMissionId?.let { dbgId ->
                    JarvisMissionDebugger.completeMission(dbgId, isSuccess, summary)
                }
            ...
            // Acquire fresh settled post-action screenshot only if vision was required for this step
            if (isVisionModel && isCaptureActive && screenshotDecision.shouldCapture) {
                Log.i(TAG, "[CAPTURE_STARTED] Capturing post-action settled screen for $activeModelName")
                try {
                    val postBase64 = captureMgr?.captureAdaptiveScreenshotBase64(
                        quality = screenshotDecision.recommendedQuality,
                        maxWidth = screenshotDecision.recommendedTargetWidth,
                        maxWaitMs = 300L
                    ) ?: captureMgr?.captureScreenshotBase64(maxWaitMs = 300L)
            ...
            val verifyLatency = System.currentTimeMillis() - verifyStartTime
            performanceTracker.recordVerificationLatency(verifyLatency)
            performanceTracker.recordActionLatency(executionResult.durationMs)
            ...
            // Phase 19: Record 18-element step trace into JarvisMissionDebugger
            activeDebuggerMissionId?.let { dbgId ->
                val worldId = "WS-" + Integer.toHexString(preObservation.screenHash.hashCode()).take(6)
                val activeGoalDesc = mission.currentSubgoal?.description ?: mission.originalUserGoal
                val activeModelName = appSettings.getActiveCustomModel()?.modelId?.takeIf { it.isNotBlank() } ?: appSettings.aiModelName
                val stepTrace = MissionStepTrace(
                    stepIndex = mission.modelDecisionCount,
                    goal = activeGoalDesc,
                    observe = "package=${preObservation.foregroundPackage}, nodes=${preObservation.nodes.size}, editable=${preObservation.editableNodes.size}",
                    worldStateId = worldId,
                    modelRequest = "Subgoal='$activeGoalDesc', Nodes=${preObservation.nodes.size}",
                    modelProvider = appSettings.aiProvider.name,
                    model = activeModelName,
                    toolSchema = CanonicalTools.ALL_DEFINITIONS.map { it.name }.toString(),
                    modelDecision = "EXECUTE_TOOL (Thought='${toolCall.thought ?: "Proceeding with action"}')",
                    toolCall = "${toolCall.name}(${toolCall.arguments})",
                    action = "Dispatched ${toolCall.name} on ${uiActionResult.target ?: "screen"}",
                    actionResult = "success=${executionResult.success}, latency=${executionResult.durationMs}ms",
                    waitDurationMs = 500L,
                    verify = "Verify transition for ${toolCall.name}",
                    verificationResult = "[${verification.state}] ${verification.explanation}",
                    nextObservation = "package=${postObservation.foregroundPackage}, nodes=${postObservation.nodes.size}",
                    replan = if (isSuccess) "None (On track)" else "Replanning evaluated",
                    finalResult = if (isSuccess) "STEP_VERIFIED" else "REPLAN_TRIGGERED",
                    failureReason = if (!isSuccess) verification.explanation else null,
                    tokenUsage = 180 + (preObservation.nodes.size * 10),
                    latencyMs = executionResult.durationMs
                )
                JarvisMissionDebugger.recordStep(dbgId, stepTrace)
            }
        ...
        if (mission.status == AutonomousMissionStatus.COMPLETED || (mission.currentSubgoal == null && mission.remainingObjectives.isEmpty())) {
            ...
            activeDebuggerMissionId?.let { dbgId ->
                JarvisMissionDebugger.completeMission(dbgId, true)
            }
            ...
            val failMsg = mission.lastActionResult?.errorMessage ?: mission.statusMessage ?: "Mission failed"
            archiveCurrentMission(MissionOutcome.FAILED, failMsg)
            activeDebuggerMissionId?.let { dbgId ->
                JarvisMissionDebugger.completeMission(
                    missionId = dbgId,
                    success = false,
                    failureReason = failMsg,
                    failedStepIndex = mission.modelDecisionCount
                )
            }
        }
    }
